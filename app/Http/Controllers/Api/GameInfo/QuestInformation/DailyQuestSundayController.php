...
class DailyQuestSundayController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($info->image && Storage::exists('public/dailyquestsunday/' . $info->image)) {
                Storage::delete('public/dailyquestsunday/' . $info->image);
            }

            $filename = time() . '_' . $request->file('image')->getClientOriginalName();
            $path = $request->file('image')->storeAs('public/dailyquestsunday', $filename);
